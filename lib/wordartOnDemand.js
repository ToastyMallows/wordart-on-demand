...
async function generateWordArt(word, art) {
	...
	// set background color of Chrome to be transparent
	...
	const tempWordFile = path.resolve(path.normalize(`${tempDir}${path.sep}temp_${getTimestamp()}.png`));
	...
	const wordFileName = `wordart-${art}-${slugify(word).substring(0, 20)}.png`;
